...
public class Problem11
{
  ...
  public static int calcHorizontal(byte[][] square)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      int product = arrayProduct(square[i]);
      if (product > max)
      {
        max = product;
      }
  ...
  public static int calcVertical(byte[][] square)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      byte[] v = {square[0][i], square[1][i], square[2][i], square[3][i]};
      int product = arrayProduct(v);
      if (product > max)
      {
        max = product;
      }
  ...
  public static int getSquareProduct(byte[][] square)
  {
    ...
    int vertical = calcVertical(square);
  ...
  public static ArrayList<byte[][]> parseSquares(byte[][] grid)
  {
    ...
    for (int r = 0; r <= grid.length - 4; r++)
    {
      ...
      {
        byte[][] square = new byte[4][4];
        for (int i = 0; i < 4; i++)
        {
          for (int j = 0; j < 4; j++)
          {
            square[i][j] = grid[r + i][c + j];
          }
        }
        squares.add(square);
      }
  ...
  public static int eulerProblem11()
  {
    int max = -1;
    for (byte[][] square : parseSquares(gridMatrix))
    {
      int product = getSquareProduct(square);
      if (product > max)
      {
        max = product;
      }
